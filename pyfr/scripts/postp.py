...
def process_convert(args):
    with H5FileIO(args.solnf) as F:
        # Get writer instance by specified type or outf extension
        if args.type:
            writer = get_writer_by_name(args.type,F.getSolution(args.soln),divisor=args.divisor,outf=args.outf,precision=args.precision)
        else:
            extn = os.path.splitext(args.outf.name)[1]
            writer = get_writer_by_extn(extn,F.getSolution(args.soln),divisor=args.divisor,outf=args.outf,precision=args.precision)
...
#    ap_unpack.set_defaults(process=process_unpack)
    ...
    ap_conv = sp.add_parser('convert', help='convert --help', description=
                            'Converts .pyfr solution for visualisation '
                            'in external software.')
    ap_conv.add_argument('solnf', help='PyFR file ')
    ap_conv.add_argument('soln', help='solution name')
